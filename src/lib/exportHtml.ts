...
export function exportProjectHtml(project: Project, contacts: Contact[], feedbackEmail = "", nextActionItems: { text: string; source?: string }[] = [], nextActionsSummary?: string | null): void {
  ...
  const nextActionsSummaryHtml = nextActionItems.length > 0 ? `
      <div style="display:flex;align-items:center;justify-content:space-between;margin-bottom:12px"><div style="font-size:10px;font-weight:700;text-transform:uppercase;letter-spacing:.08em;color:#6B7280">Coming up next</div>${feedbackPill(project.title, "Coming up next", feedbackEmail)}</div>
