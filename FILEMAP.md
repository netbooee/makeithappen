...
| `src/pages/ExecutiveUpdate.tsx` | Portfolio Executive Update page — Modernist-styled roll-up across every active project (health, risk, budget, schedule, editable summary poster, matrix, per-project detail), plus the per-project ExecEntry data layer used to build the portfolio report |
...
| `src/lib/exportExecUpdateHtml.ts` | Static HTML export of the Modernist portfolio report shown on the Executive Update page — mirrors its bands (title, metrics, summary poster, all-projects matrix, project detail, footer), always full detail, built from the shared `PortfolioSummary` in `portfolioReport.ts` |
| `src/lib/portfolioReport.ts` | Pure computation for the Modernist portfolio view in `ExecutiveUpdate.tsx` — RAG roll-ups, phase stepper, budget/schedule aggregation, composed (templated, not AI) narrative bands, and the poster `summary` (manual override when set, else auto-composed) |
